  ...
  <div class="newsPress">
        ...
        <span class="newsPressAside">
            <span @click="handleIsType(null)">
                <p :class="isType === null ? 'newsTypeActive' : ''">{{$i18n.localeProperties.code === "ko" ? "전체" : "All"}}</p>
            ...
            <span v-for="(data, idx) of typeList"
                ...
                <p :class="isType === data.id ? 'newsTypeActive' : ''">{{$i18n.localeProperties.code === "ko" ? data.name : data.nameEn}}</p>
                <i><img src="https://velog.velcdn.com/images/kyj0206/post/677e0a5b-146e-46a5-a5db-ef2b185febf4/image.png"
                        alt="newsPressTag"></i>
            </span>
        </span>
        ...
        <div class="newsPressMbAside select">
            <div class="selected"
                ...
                <div class="selected-value">{{isType ? $i18n.localeProperties.code === "ko" ? typeList[typeList.findIndex(e => e.id === isType)].name : typeList[typeList.findIndex(e => e.id === isType)].nameEn : $i18n.localeProperties.code === "ko" ? "전체" : "All"}}</div>
                <div class="arrow"></div>
              </div>
              <ul v-if="isToggle">
                <li class="option"
                    @click="[handleIsType(null),isToggle = !isToggle]">{{$i18n.localeProperties.code === "ko" ? "전체" : "All"}}</li>
                <li class="option"
                    v-for="(data, idx) of typeList"
                    :key="idx"
                    @click="[handleIsType(data.id),isToggle = !isToggle]">{{$i18n.localeProperties.code === "ko" ? data.name: data.nameEn}}</li>
              </ul>
        </div>
        <!-- 블로그 탭 리스트 -->
...
export default {
    ...
    async asyncData ({$axios}) {
      try {
        const data = Promise.all([$axios.get(`admin/news/type`),$axios.get(`admin/news?page=1&size=${20}`)]);
...
@media screen and (max-width: 1024px) {
    ...
    .newsPress .tabCont .newsPressList .tutorialBox .tutorialBoxCont > div:nth-child(2){
        margin-right: 0;
    }

    .newsPress .tabCont .newsPressList .userBox .userBoxCont > div:nth-child(3),
    .newsPress .tabCont .newsPressList .tutorialBox .tutorialBoxCont > div:nth-child(3),
    .newsPress .tabCont .newsPressList .guideBox .guideBoxCont > div:nth-child(3){
        margin-right: 15px;
    }
...
@media screen and (max-width: 650px) {
    ...
    .newsPress .tabCont .newsPressList .userBox .userBoxCont > div a span.listInfo span:nth-child(2){
        margin-bottom: 5px;
    }
    .tutorialBoxCont>div {
      margin-bottom: 15px;
